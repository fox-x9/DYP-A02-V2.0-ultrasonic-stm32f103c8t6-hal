...
/**
  * @brief  UART接收完成回调 (HAL库中断方式)
  */
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1) {
        /* 超声波传感器: 逐字节解析 */
        Ultrasonic_ParseByte(ultra_rx_byte);
        /* 继续下一次接收 */
        HAL_UART_Receive_IT(&huart1, &ultra_rx_byte, 1);
    }
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (g_ultra.data_ready) {
      ...
      /* 通过USART2输出距离 (格式: "Dist: XXXX mm\r\n") */
      int len = snprintf(u2_tx_buf, sizeof(u2_tx_buf),
                         "Dist: %u mm\r\n", dist);
